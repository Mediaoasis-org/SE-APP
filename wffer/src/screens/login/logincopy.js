		...
		// console.log(this.state.dataSource)
			this.state.dataSource.map((item,index)=>{
				 let textInput = this.state.textInput;
				    textInput.push(<TextInput key={index+1} name={item.name} placeholder={item.label} />);
				    this.setState({ textInput })
			})
			alert(JSON.stringify(this.state.textInput))
	  }
	  else
	  {
						...
						<View>
								{
							    	...
							    	this.state.dataSource.map((item)=>{
										...
										if(item.type=='Submit'){
											return (
											<View>
													<TouchableOpacity onPress={()=>alert(JSON.stringify(this.state.textInput))} style={gstyles.buttonView}>
...
// <TouchableOpacity onPress={()=>alert(this.state.textInput)} style={gstyles.buttonView}>
